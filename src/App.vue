...
provide("incorrectMode", incorrectMode);
</script>
...
<template>
  ...
  >
    <div class="w-full max-w-7xl mx-auto p-4 flex flex-col items-center">
      <header
        class="w-full flex justify-between items-center mb-2 rounded-2xl shadow-xl bg-gray-900/80 backdrop-blur-md px-6 py-4 sticky top-0 z-10"
      >
        <h1
          class="text-3xl md:text-4xl font-extrabold tracking-tight drop-shadow-sm break-words w-full"
        >
          <router-link to="/" class="hover:text-blue-400 transition-colors">
        ...
        <BurgerMenu>
          <template v-if="route.path === '/'">
            ...
            <button
              ...
              class="w-full flex items-center gap-2 p-2 rounded-lg text-gray-300 hover:bg-gray-700 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400"
            ...
            <!-- 4. Incorrect Words Mode Toggle -->
            <button
              @click="incorrectMode = !incorrectMode"
              class="w-full flex items-center gap-2 p-2 rounded-lg text-gray-300 transition-all duration-200 focus:outline-none focus:ring-2"
              :title="
                incorrectMode ? 'Disable Incorrect Words Mode' : 'Enable Incorrect Words Mode'
              "
            >
              <span class="flex items-center">
                <span
                  :class="[
                    'relative inline-block w-10 h-6 transition duration-200',
                    incorrectMode ? 'bg-blue-700' : 'bg-gray-600',
                  ]"
                  style="border-radius: 9999px"
                >
                  <span
                    :class="[
                      'absolute left-1 top-1 w-4 h-4 bg-white rounded-full shadow transform transition duration-200',
                      incorrectMode ? 'translate-x-4' : '',
                    ]"
                  ></span>
                </span>
              </span>
          ...
          <template v-else-if="route.path === '/stats'">
            <button
              @click="goBack"
              class="w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-transparent bg-gray-800/80 text-gray-200 font-semibold shadow-sm hover:bg-gray-700 hover:border-gray-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400"
            >
      ...
      <div
        v-if="route.path === '/'"
        class="w-full text-lg text-gray-500 font-semibold flex items-center justify-center"
      >
        <span
